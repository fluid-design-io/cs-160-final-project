...
const MenuListItem: React.FC<MenuListItemProps> = ({ menu, pageRef }) => {
  ...
  const [present, dismiss] = useIonModal(MenuListModal, {
    onDismiss: () => dismiss(),
    menu,
  });
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col justify-center items-center">
                <label className="flex h-5 w-5 cursor-pointer flex-col items-center justify-center rounded-full bg-primary-tint uppercase tracking-wide">
                  <span className="text-base text-[white] dark:text-[black] leading-normal">
                    S
                  </span>
                </label>
...
const MenuListModal = ({ menu, onDismiss }) => {
  return (
    <IonPage>
      <IonHeader>
        <IonToolbar>
          <IonTitle>{menu.menuName}</IonTitle>
